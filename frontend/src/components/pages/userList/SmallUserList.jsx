...
export default function SmallUserList(props) {

    const updatedUsers = props.updatedUsers;
        ...
        return(
            <div className="actionCell">
                <HighlightOffIcon
                    ...
                    onClick={ () =>
                        handleDeleteButton(projectData, row)
                }/>
            </div>
        )
    }

    function handleDeleteButton(projectData, row) {
        axios.delete('http://localhost:8000/api/remove-user-from-project/'+projectData.id, {data : row})
        .then((res) => {
            setTitleSuccess();
            setDescSuccess();
            modalOpen();
    ...
    function handleAddButton(projectData, row) {
        axios.put('http://localhost:8000/api/add-user-to-project/'+projectData.id, row)
        .then((res) => {
            setTitleSuccess();
            setDescSuccess();
            modalOpen();
        ...
        return(
            <div className="actionCell">
                <AddCircleOutlineIcon
                    ...
                    onClick={ () =>
                        handleAddButton(projectData, row)
                }/>
            </div>
        )
    }
