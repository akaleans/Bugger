...
export default function TicketAttachmentList(props) {
    ...
    const [rows, setRows] = useState([
        { id: 1, submitter: '', file: '', notes: '', date_created: '', ticket_name:'' }
    ]);
    const [dataLoaded, setDataLoaded] = useState("");
    const [data, setData] = useState(rows)

    useEffect(() => {
        axios.get('http://localhost:8000/api/get-ticket-attachments/'+ticketData.id)
        .then((res) => {
            setDataLoaded(true);
            setData(res.data)
            setRows(res.data);
    ...
    const columns = [
        ...
        { field: 'notes', headerName: 'Notes', flex: 2, renderCell: (params) => {
            const str = Capitalize(params.row.notes);
            return (
                <div className="messageContainer">
                    <Link to={{
                            pathname: "/ticket-attachment/"+params.row.id,
                            state: {
                                id: params.row.id,
                            }}}
                        >
